                        ...
                        if codon in findORFs.stopCodons:
                                ...
                                if self.reverseStarts[framePosition] != []:
                                        ...
                                        if geneLength > 100:
                                                ...
                                                self.GeneSeq.append(revComp[startPosition:stopPosition])
                                        #reset the frame        
                                        self.reverseStarts[framePosition] = []
                                        
                return (self.ORFlist, self.GeneSeq)
                ...
                for i in range (0, len(OriginalSeq)):
                        ...
                        if codon in findORFs.stopCodons:
                                ...
                                if self.forwardStarts[framePosition] != []:
                                        ...
                                        startPosition = self.forwardStarts[framePosition][0]
                                        stopPosition = i + 2
                                        geneLength = stopPosition - startPosition
                                        
                                        if geneLength > 100:
                                                #add to the list of ORFs#add 1
                                                self.ORFlist.append([startPosition +1 , stopPosition +1, geneLength + 1, (framePosition + 1)])
                                                self.GeneSeq.append(OriginalSeq[startPosition:stopPosition])
                                        #reset the frame        
...
def main():
        ...
        orfOutput = open('./FileORFsList.txt', "w")
        GeneOutput = open('./genelist.txt', "w")
        ...
        seqRead = sequenceAnalysis.FastAreader(FileName)
        ...
        for header, Sequence in seqRead.readFasta():
                ...
                #print: frame, start position, end position, length
                        ...
                        print('>{:+d} {:>5d}..{:>5d} {:>5d}'.format(ORF.ORFlist[i][3], ORF.ORFlist[i][0], ORF.ORFlist[i][1], ORF.ORFlist[i][2]), file = GeneOutput)
                        print(ORF.GeneSeq[i], file = GeneOutput)
